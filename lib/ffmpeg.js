 ...
 */
function extractAudioTrack(inputFile, trackIndex, outputFile) {
    ...
    return new Promise((resolve, reject) => {
        ...
        exec('mkvextract --version', (error) => {
            if (!error) {
                ...
                exec(mkvExtractCmd, (extractError, stdout, stderr) => {
                    if (extractError) {
                        console.log('⚠️  mkvextract falló, intentando con ffmpeg...');
                        extractWithFFmpeg(inputFile, trackIndex, outputFile)
                            .then(resolve)
                            .catch(reject);
                    } else {
 ...
 */
function extractWithFFmpeg(inputFile, trackIndex, outputFile) {
 ...
 */
function cleanAudio(inputAudio, outputAudio, bitrate = 192) {
